...
def admin_each_station_regular_revenue_by_date_type(request):
    ...
    for station in stations:
        # Ensure all categories are present, even if there's no revenue
        if selected_date_type == "weekly":
            categories = [
                "Monday",
                "Tuesday",
        ...
        revenue_list = [
            station_revenues[station.id].get(category, 0) for category in categories
        ]
        data.append(sum(revenue_list))  # Summing up all categories for each station
