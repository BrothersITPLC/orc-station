...
def admin_each_station_weight_by_date_type(request):
    ...
    # Initialize data structure for station weights
    station_weights = {station.id: {} for station in stations}
    ...
    for checkin in checkins:
        ...
        # Categorize weight based on selected_date_type
        if selected_date_type == "weekly":
            day_of_week = checkin.checkin_time.strftime("%A")
            station_weights[station_id][day_of_week] = (
                station_weights[station_id].get(day_of_week, 0) + weight
            )
        elif selected_date_type == "monthly":
            week_number = (checkin.checkin_time.day - 1) // 7 + 1
            week_label = f"Week {week_number}"
            station_weights[station_id][week_label] = (
                station_weights[station_id].get(week_label, 0) + weight
            )
        elif selected_date_type == "yearly":
            month_name_str = checkin.checkin_time.strftime("%B")
            station_weights[station_id][month_name_str] = (
                station_weights[station_id].get(month_name_str, 0) + weight
            )
        else:
            return Response(
                {
    ...
    # Build response data
    labels = [station.name for station in stations]
    data = []

    for station in stations:
        # Ensure all categories are present, even if there's no weight
        if selected_date_type == "weekly":
            categories = [
                "Monday",
                "Tuesday",
        ...
        weight_list = [
            station_weights[station.id].get(category, 0) for category in categories
        ]
        data.append(sum(weight_list))  # Summing up all categories for each station
