...
def controller_tax_payers_registered_by_date_type(request):
    ...
    missing_params = [
        param
        for param in ["selected_date_type", "start_date", "end_date", "controller_id"]
    ...
    # Filter taxpayers by controller_id and date range
    taxpayers = Exporter.objects.filter(
    ...
    categories = []
    type_data = defaultdict(lambda: [0] * (7 if selected_date_type == "weekly" else 12))
    ...
    if selected_date_type == "weekly":
        ...
        grouped_data = (
            taxpayers.annotate(day_of_week=ExtractWeekDay("created_at"))
            .values("day_of_week", "type__name")
        ...
        for entry in grouped_data:
            taxpayer_type = entry["type__name"]
            index = categories.index(day_to_name[entry["day_of_week"]])
            type_data[taxpayer_type][index] = entry["count"]

    elif selected_date_type == "monthly":
        grouped_data = (
            taxpayers.annotate(week_number=ExtractWeek("created_at"))
            .values("week_number", "type__name")
            .annotate(count=Count("id"))
        )
        ...
        grouped_data = (
            taxpayers.annotate(month=ExtractMonth("created_at"))
            .values("month", "type__name")
            .annotate(count=Count("id"))
        )
    ...
    # Convert the dynamic dictionary into a list of series
    series = [
        {"name": taxpayer_type, "data": data}
        for taxpayer_type, data in type_data.items()
    ]
